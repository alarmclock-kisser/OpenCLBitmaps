...
{
    public class ClContextHandling
    {
        // ----- ----- ----- ATTRIBUTES ----- ----- ----- \\
        ...
        public ClMemoryHandling? MemoryH = null;
		public ClKernelHandling? KernelH = null;
        ...
        // ----- ----- ----- CONSTRUCTOR ----- ----- ----- \\
        public ClContextHandling(string repopath, ListBox logListbox, ComboBox devicesCombobox)
        {
            // Set attributes
            this.Repopath = repopath;
            this.LogList = logListbox;
            this.DevicesCombo = devicesCombobox;

            // Register events
            this.DevicesCombo.SelectedIndexChanged += (s, e) => this.InitContext(this.DevicesCombo.SelectedIndex);
        ...
        // UI methods
        public void FillDevicesCombo(int selection = -1)
        {
            // Get all OpenCL devices & platforms
            Dictionary<CLDevice, CLPlatform> devicesPlatforms = this.DevicesPlatforms;
            ...
            foreach (KeyValuePair<CLDevice, CLPlatform> device in devicesPlatforms)
            {
                ...
                this.DevicesCombo.Items.Add(deviceName + " (" + platformName + ")");
            }

            // Select selection if valid
            if (selection >= 0 && selection < this.DevicesCombo.Items.Count)
            {
                this.DevicesCombo.SelectedIndex = selection;
            }
            else
            {
                this.DevicesCombo.SelectedIndex = -1;
            }
        }
        ...
        public void InitContext(int index = 0, bool silent = false)
        {
            ...
            this.MemoryH = new ClMemoryHandling(this.Repopath, this.LogList, this.CTX.Value, this.DEV.Value, this.PLAT.Value);
			this.KernelH = new ClKernelHandling(this.Repopath, this.MemoryH, this.LogList, this.CTX.Value, this.DEV.Value, this.PLAT.Value, this.MemoryH.QUE);
