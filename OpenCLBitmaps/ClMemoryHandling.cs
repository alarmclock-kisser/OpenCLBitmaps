...
namespace OpenCLBitmaps
{
    public class ClMemoryHandling
    {
        ...
        // ----- ----- ----- CONSTRUCTOR ----- ----- ----- \\
        public ClMemoryHandling(string repopath, ListBox logList, CLContext ctx, CLDevice dev,CLPlatform plat)
        {
            // Set attributes
		...
		// Free buffer
		public long FreeBuffer(long pointer, bool readable = false)
        {
        ...
        // Buffer info
        public Type GetBufferType(long pointer)
        {
            // Get single buffer, if null, get array buffers
            CLBuffer? buffer = this.GetSingleBuffer(pointer, out IntPtr length);
            if (buffer != null)
		...
		// Single buffer
		public CLBuffer? GetSingleBuffer(long pointer, out IntPtr length)
        {
            // Init. length
            length = new IntPtr(0);
        ...
        public long PushData<T>(T[] data) where T : unmanaged
        {
            // Check data
        ...
        public T[] PullData<T>(long pointer, bool keep = false) where T : unmanaged
        {
            // Get buffer & length
            CLBuffer? buffer = this.GetSingleBuffer(pointer, out IntPtr length);
        ...
        public long AllocateSingle<T>(IntPtr size) where T : unmanaged
        {
            // Check size
            if (size.ToInt64() < 1)
            {
        ...
        public long PushChunks<T>(List<T[]> chunks) where T : unmanaged
        {
            ...
            {
                buffers[i] = CL.CreateBuffer(this.CTX, MemoryFlags.AllocHostPtr | MemoryFlags.ReadWrite, chunks[i], out CLResultCode error);
                if (error != CLResultCode.Success)
                {
