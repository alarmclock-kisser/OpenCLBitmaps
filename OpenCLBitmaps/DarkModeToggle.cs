...
namespace OpenCLBitmaps
{
	public static class DarkModeToggle
	{
		...
		{
			// Verify window
			window ??= MainForm ?? Application.OpenForms[0];
			if (window == null)
			{
				return;
			}
			...
			// Determine new mode
			IsDarkMode = forceDarkMode ?? window.BackColor.GetBrightness() < 0.5f;

			// Apply to main form
			ApplyColors(window, IsDarkMode);

			// Recursively apply to all child controls
			ApplyToControlCollection(window.Controls, IsDarkMode);
		}
		...
		private static void ApplyToControlCollection(Control.ControlCollection controls, bool darkMode)
		{
			...
			{
				ApplyColors(control, darkMode);

				// Recursive call for container controls
				if (control.HasChildren)
				{
					ApplyToControlCollection(control.Controls, darkMode);
				}
			}
		}
		...
		private static void ApplyColors(Control control, bool darkMode)
		{
			...
			Dictionary<Type, Color> palette = darkMode ? _darkPalette : _lightPalette;
		...
		{
			// Verify listBox_log
			listBox_log ??= FindControls<ListBox>(MainForm, "listBox_log", true, true).FirstOrDefault();
			if (listBox_log == null)
			{
				return null;
			}
